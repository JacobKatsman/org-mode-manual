...
+ C-c C-x C-w (M-x org-cut-subtree)   - kill (вырезать) подуровень. С
...
+ C-c c-x c-o/x (M-x org-clock-out/org-clock-in-last) - stop/cancel clock on current item 8.4.2 THE CLOCK TABLE PASSIVE
