#+STARTUP: showall
#+TITLE: "Org-mode short usefull manual"
#+AUTHOR: "J/Z/Katsman"
#+EMAIL:  "call89269081096@gmail.com"
#+INCLUDE: "C:\\org-mode\\header.org"

* Предварительные настройки (Getting Started)
...
The point is that Org-mode requires simultaneous setting .emacs file, setting org-mode file,
...
- Первое замечание: Emacs расчитан на скорость печати в английской раскладке более 50 WPM
- Второе замечание: В Emacs невозможно, да и не нужно, работать без "доски"!

** Это шапка нашего org-mode файла (её включаем INCLUD-ом):
...
** Настройка. Обязательный префикс в начале файла 

Emacs does not actually understand you are editing an Org document,
yet. To enable Org mode on your current document, 
...
+ M-shift-RET (org-insert-todo-heading) Создать новую задачу-заголовок.
+ M-RET - Вставить задачу-заголовок того же уровня, что и текущий. 

+ C-c C-x C-w (org-cut-subtree)   - kill (вырезать) подуровень. С
  числовым аргументом *C-u C-c C-x C-w*  N убивает N подуровней.
+ C-c C-x M-w (org-copy-subtree)  - копировать подуровень. С числовым аргументом N копирует N подуровней.
+ C-c C-x C-y (org-paste-subtree) - вставить подуровень. Уровень вставляемого блока приводится в соответствие с тем местом в которое он копируется.
...
+ C-c C-q (org-set-tags-command)  -  There is also a special command for inserting tags:
+ C-c C-c – add tag
...
+ C-c С-z (org-add-note) - поставить комментарий к задаче 
+ C-с C-с (org-ctrl-c-ctrl-c)  - сохранить комментарий к задаче
...
+ C-30 C-c C-w  - показать все дедлайны за 30 дней
...
+ Так выглядят задачи *в интервале*: 
#+BEGIN_EXAMPLE 
<00-00>--<00-00>
#+END_EXAMPLE
...
** Таймер 
+ C-c c-x c-i (org-clock-in) - start clock on current item 8.4.2 THE CLOCK TABLE ACTIVE
+ C-c c-x c-o/x (org-clock-out/org-clock-in-last) - stop/cancel clock on current item 8.4.2 THE CLOCK TABLE PASSIVE
+ C-u C-c C-y - посчитать время интервала
+ C-c . просто проставить текущую дату
+ C-u c-c .  +1d 12:00 - в данном случае это переодический интервал  
...
#+BEGIN_EXAMPLE
 ...
 #+END:
#+END_EXAMPLE
3. Вызвать пересчет часов *M-x org-clock-report* - создадим репорт по всем задачам

* Параметры показа блоков (Global and local cycling)
+ *C-u C-u C-u TAB (outline-show-all)* Показать все. expand all trees Emacs and org mode
...
* Ссылки и includ-ы в org-mode  (Handling Links)
...
Выделение шрифтом текста в org-mode:
...
_M-x org-display-inline-images_ показать картинки 

* Архивирование записей (Moving a tree to an archive file). 
...
* Как в стилевой файл org-mode включить исполняемый код?
#+BEGIN_SRC
...
* Дополнительно 
 + С-x r t (M-x string-rectangle)  - вставим префикс-колонку метки до
   ...
   C-x r r [A] (copy-rectangle-to-register)- скопируем стобец
   C-x r i [A] (insert-register)- вставим столбец
...
#+END_SRC
** Справочная функция 
C-h k <C-x C-y C-k>  какая функция отвечает за данную комбинацию клавиш
** Редактируем  ~/.emacs   
...
+ https://channikhabra.com/blog/managing-chaos-org-mode-as-a-todo-list/
